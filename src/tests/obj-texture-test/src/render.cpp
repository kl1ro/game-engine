...
void drawSubmesh(const Submesh& submesh) {
	...
	bool materialExists = submesh.materialIndex > -1
		&& submesh.materialIndex < (int)Globals::materials.size();

	if (ctx.mode == GL_LINE || !materialExists) drawSubmeshWithContext(submesh); 
	else drawSubmeshWithMaterial(submesh);
}
