...
void loadMaterials(
	...
	if (tinyMaterials.empty() && Globals::materials.empty()) {
		Globals::materials.push_back({
...
void loadObj(const std::string& path) {
	...
	for (auto& shape : shapes) {
			...
			for (int f = 0; f < (int)shape.mesh.material_ids.size(); f++) {
					...
					if (matId < 0) matId = 0; // fallback for faces with no material
					
					Submesh& submesh = submeshMap[matId];
					submesh.materialIndex = materialOffset + matId;
					
					for (int v = 0; v < 3; v++) {
							tinyobj::index_t idx = shape.mesh.indices[faceIndex + v];
			...
			for (auto& [matId, submesh] : submeshMap) {
					glGenVertexArrays(1, &submesh.buffers.VAO);
					glGenBuffers(1, &submesh.buffers.VBO);
					glGenBuffers(1, &submesh.buffers.EBO);
					glBindVertexArray(submesh.buffers.VAO);
					glBindBuffer(GL_ARRAY_BUFFER, submesh.buffers.VBO);
					glBufferData(GL_ARRAY_BUFFER, submesh.vertices.size() * sizeof(float), submesh.vertices.data(), GL_STATIC_DRAW);
					glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, submesh.buffers.EBO);
